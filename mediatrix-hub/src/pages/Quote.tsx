...
import { updateQuoteForm, submitQuoteForm } from '../store/apps/form';
import caseInfo from '../data/caseInfo.json';

const QuotePage: React.FC = () => {
  const dispatch = useDispatch<AppDispatch>();
  const { quoteType } = useParams<{ quoteType: string }>();
  const { quoteForm, status, error } = useSelector((state: RootState) => state.form);

  useEffect(() => {
    if (quoteType) dispatch(updateQuoteForm({ quoteType: quoteType || '' }));
  }, [dispatch, quoteType]);
  ...
  const handleSubmit = (e: React.FormEvent<HTMLFormElement>) => {
    e.preventDefault();
    dispatch(submitQuoteForm(quoteForm));
  };
  ...
  return (
    <div className="bg-[#0a0f1c] min-h-screen flex items-center justify-center p-6">
      <div className="bg-[#1c2331] p-8 rounded-lg shadow-lg w-full max-w-md border border-[#4299e1]">
        ...
        <form onSubmit={handleSubmit} className="space-y-6">
          ...
          <button
            type="submit"
            className="w-full bg-black text-white p-3 rounded-md hover:bg-[#2d3748] transition-colors"
